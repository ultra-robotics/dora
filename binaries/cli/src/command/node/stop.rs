...
impl Executable for StopNode {
    fn execute(self) -> eyre::Result<()> {
        ...
        let reply_raw = session
            .request(
                &serde_json::to_vec(&ControlRequest::StopNode {
                    dataflow_uuid: self.dataflow,
                    node_id: self.node_id.into(),
                })
                .unwrap(),
            )
            .wrap_err("failed to send stop node request")?;
        ...
        match reply {
            ControlRequestReply::NodeStopped {
                ...
                println!("Node {node_id} stopped in dataflow {dataflow_uuid}");
                Ok(())
            }
            ControlRequestReply::NodeStopError { error } => bail!("Failed to stop node: {error}"),
